...
const Todo = () => {
 ...
 const submit = async () => {
  ...
  if (data.id) {
   console.log(data);
   await api.patch(`/todos/${data.id}`, tmp).catch((err) => console.log(err));
  } else {
   tmp.userid = userSelector.id;
   console.log(tmp);
   await api.post('/todos', tmp).catch((err) => console.log(err));
  }
  await fetchTasks(userSelector.id);
    ...
    <>
     ...
     <div className="bg">
      ...
      <div className="center full col gap-10">
       <Elipse />
       <div className="semibold white">Welcome {userSelector.fullname}</div>
      </div>
     </div>

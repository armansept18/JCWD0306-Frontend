...
const Login = ({ users = [] }) => {
 ...
 const login = async () => {
  ...
  if (!auth.data.payload.length) return alert('email/password salah');

  delete auth.data.payload.password;

  dispatch({
   type: types.login,
   payload: { ...auth.data.payload[0] }
  });

  localStorage.setItem('auth', JSON.stringify(auth.data.payload[0]));
  alert('hello');
  nav('/dashboard');
 };
 return (
